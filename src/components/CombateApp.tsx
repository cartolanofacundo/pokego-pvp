...
// `updatedAt` no es parte del estado en memoria (no dispara recálculos): se
// suma solo al guardar, para que Inicio pueda mostrar "hace N días". Vive
// aparte de `Persisted` porque `EMPTY`/`demoState` no lo necesitan.
type PersistedOnDisk = Persisted & { updatedAt: number };
...
export function CombateApp() {
  ...
  useEffect(() => {
    if (ready) saveJSON(STORAGE_KEY, { ...state, updatedAt: Date.now() } satisfies PersistedOnDisk);
  }, [ready, state]);
